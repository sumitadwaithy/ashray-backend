...
interface AgreementData {
  ...
  nocSerial?: string;
  nocDate?: string;          // date of NOC issue
  transferNote?: string;
}
...
export const EnglishPostSaleNOC: React.FC<{ data: AgreementData }> = ({ data }) => {
...
<div className="text-center text-[13px] font-semibold mt-1 mb-6 text-gray-600">
          (Post-Sale — Full &amp; Final Payment Received — Zero Dues Clearance)
        </div>
  ...
  <div className="whitespace-nowrap text-right">
    ...
    <span>{data.property.bookingDate}</span>
        ...
        <div className="clause" style={{ marginBottom: '14px' }}>
          <strong>Subject:</strong> No Objection Certificate — Full and Final Payment Received — Zero Dues Clearance for Plot No. <span className="underline-blank">{safe(data.property.plotNumber)}</span>, <span className="underline-blank">{safe(data.property.projectName)}</span> Project, District <span className="underline-blank">{safe(data.property.district)}</span>.
        </div>
        ...
        <table className="property-table">
          <tbody>
            ...
            <tr><td>Total Sale Consideration</td><td>₹ {formatAmount(data.property.totalAmount)} /- Only</td></tr>
            <tr><td>Payment Mode</td><td>{safe(data.property.paymentMode)}</td></tr>
            <tr><td>Payment Reference No.</td><td>{safe(data.property.paymentReference)}</td></tr>
            <tr><td>Date of Final Payment</td><td>{formatDate(issueDate)}</td></tr>
          </tbody>
        </table>
        ...
        <div className="clause">
          ...
          This is to certify that {companyDisplay} has received the <strong>complete, full, and final sale consideration</strong> of ₹ {formatAmount(data.property.totalAmount)}/- (Rupees {safe(data.property.totalAmount)} Only) from <strong>{clientName}</strong> in respect of Plot No. {safe(data.property.plotNumber)}, measuring {safe(data.property.area)} Sq. Ft., situated in the <strong>{safe(data.property.projectName)}</strong> project, Mouza {safe(data.property.locality)}, Tehsil {safe(data.property.tehsil)}, District {safe(data.property.district)}, as on {formatDate(issueDate)}.
        </div>

        <div className="clause">
          <span className="clause-num">2. </span>
          {companyDisplay} hereby unequivocally confirms and declares that as on {formatDate(issueDate)}, there are <strong>NO OUTSTANDING DUES, NO PENDING PAYMENTS, NO BALANCE AMOUNTS, AND NO FINANCIAL OBLIGATIONS</strong> of any nature whatsoever remaining from {clientName} towards {companyDisplay} in respect of the above-mentioned property. The account between both parties stands fully settled and closed.
        </div>

        <div className="clause">
          <span className="clause-num">3. </span>
          {companyDisplay} has <strong>absolutely no objection</strong> to {clientName} proceeding with the registration of the above plot in their name and those of their nominees, heirs, or legal representatives, before the concerned Sub-Registrar or any other competent authority, and {companyDisplay} undertakes to provide all cooperation required for such registration without any further claim, demand, or condition.
        </div>

        <div className="clause">
          <span className="clause-num">4. </span>
          {companyDisplay} further confirms that {clientName} is the rightful and lawful owner of the above-described plot upon full payment of the agreed consideration, and {companyDisplay} hereby relinquishes and waives any and all claims, rights, or interests over the said property with immediate effect from the date of this certificate.
        </div>

        <div className="clause">
          <span className="clause-num">5. </span>
          {companyDisplay} hereby grants unconditional clearance to {clientName} to undertake any construction, development, transfer, mortgage, gift, or any other lawful act on the said property as the absolute owner thereof, without any interference, objection, or claim from {companyDisplay}, its directors, partners, employees, successors, heirs, or assigns.
        </div>

        <div className="clause">
          <span className="clause-num">6. </span>
          This No Objection Certificate is issued freely, voluntarily, and in good faith upon receipt of the full consideration amount, and shall be treated as final documentary evidence of complete payment and zero dues. This certificate is irrevocable, legally binding, and shall be enforceable against {companyDisplay} and its successors in all courts of law and before all competent authorities in India.
        </div>
...
export default EnglishPostSaleNOC;